...
def hf3hPN(f, M, eta, phi_c, s1x=0.0, s1y=0.0, s1z=0.0, s2x=0.0, s2y=0.0, s2z=0.0, Lam=0.0, dLam=0.0, Deff=1.0):
    ...
    Phi = Phif3hPN(f, M, eta, phi_c, s1x=s1x, s1y=s1y, s1z=s1z, s2x=s2x, s2y=s2y, s2z=s2z, Lam=Lam, dLam=dLam)
...
def compute_rf(par, h0, fbin, fbin_ind):
    ...
    Mc, eta, chieff, chia, Lam, dtc = par
    M = Mc / eta ** 0.6
    delta = np.sqrt(1.0 - 4.0 * eta)
    ...
    # waveform ratio
    r = hf3hPN(f, M, eta, 0.0, s1z=s1z, s2z=s2z, Lam=Lam) / h0_bin * np.exp(-2.0j * np.pi * f * dtc)
    r0 = 0.5 * (r[:-1] + r[1:])
    r1 = (r[1:] - r[:-1]) / (f[1:] - f[:-1])
...
def lnlike(par, sdat, h0, fbin, fbin_ind, ndtct):
    ...
    Mc, eta, chieff, chia, Lam = par[0:5]
    dtc = par[5:]
    par_notc = [Mc, eta, chieff, chia, Lam]
